...
// VERIFY PAYMENT ENDPOINT WITH DETAILED LOGGING
app.post('/api/payment/verify', async (req, res) => {
  try {
    ...
    if (!razorpay_payment_id || !razorpay_subscription_id || !razorpay_signature || !email) {
    ...
    // Verify signature
    console.log('Creating signature for verification...');
    const body = razorpay_subscription_id + '|' + razorpay_payment_id;
    console.log('Signature body string:', body);
    ...
    if (findUserError) {
      ...
      console.log('Full findUserError object:', JSON.stringify(findUserError, null, 2));
      return res.status(404).json({
        error: 'User not found',
    ...
    console.log('Updating user plan to paid and subscription status to active...');
    ...
    if (updateError) {
      console.log('=== USER UPDATE ERROR IN VERIFY ===');
      console.log('Full updateError object:', JSON.stringify(updateError, null, 2));
      console.log('Error message:', updateError.message);
      console.log('Error details:', updateError.details);
      console.log('Error hint:', updateError.hint);
